...
def _fetch_global_market_cap():
    ...
    try:
        resp = requests.get(GLOBAL_URL, headers=HEADERS, timeout=10)
        resp.raise_for_status()
...
def get_top_cryptos_full(n=10):
    ...
    it) and each coin's market-cap dominance (its share of total crypto
    market cap, fetched separately since /coins/markets doesn't include
    the total).
    """
