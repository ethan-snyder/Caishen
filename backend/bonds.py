...
def get_bond_data_full():
    """
    Data-only variant for API consumers: current yield, change vs. the
    prior close, and the term label used in the UI. Only the four tenors
    yfinance reliably covers (see module docstring) -- deliberately not
    padded out to a full curve with invented tenors.
    """
    results = []
    for name, symbol in TREASURIES.items():
        try:
