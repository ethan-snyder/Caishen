...
def get_fx_data_full():
    ...
    change, and bid/ask. yfinance doesn't reliably expose bid/ask for FX
    tickers, so when they're missing this approximates a tight spread
    around the last price (flagged via "bid_ask_is_estimate") rather than
    leaving the frontend with nothing to show.
    """
    results = []
    for base, quote, symbol in FX_PAIRS:
        try:
            ...
            results.append({
                ...
                "base_flag": CURRENCY_FLAGS.get(base), "quote_flag": CURRENCY_FLAGS.get(quote),
