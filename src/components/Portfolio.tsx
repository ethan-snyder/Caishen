  ...
  return (
    <div>
      ...
      {holdings.length === 0 ? (
        ...
        <>
          {/* Summary row */}
          <div style={{ display: 'grid', gridTemplateColumns: hasCostBasis ? 'repeat(4, 1fr)' : 'repeat(1, 1fr)', gap: 6, marginBottom: 16 }}>
            {[
              { label: 'TOTAL VALUE', value: formatMoney(data.total_value, 0), color: '#C8FFD4', show: true },
              { label: 'TOTAL COST', value: formatMoney(data.total_cost_basis, 0), color: '#4DCC88', show: hasCostBasis },
              { label: 'TOTAL GAIN', value: data.total_gain !== null ? `${data.total_gain >= 0 ? '+' : ''}${formatMoney(data.total_gain, 0)}` : '—', color: posNegColor(data.total_gain), show: hasCostBasis },
              { label: 'TOTAL RETURN', value: data.total_gain_pct !== null ? `${data.total_gain_pct >= 0 ? '+' : ''}${data.total_gain_pct.toFixed(2)}%` : '—', color: posNegColor(data.total_gain_pct), show: hasCostBasis },
            ].filter(s => s.show).map(s => (
              <div key={s.label} style={{ backgroundColor: '#060E18', border, padding: '14px 16px' }}>
